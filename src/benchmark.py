...
def gen_data_for_benchmark():
    n_node_list = list(range(15, 21, 1))
    n_vehicle_list = list(range(10, 55, 5))
    n_customer_list = list(range(300, 450, 25))
...
def benchmarking():
    ...
    length = len(n_node_list) + len(n_vehicle_list) + len(n_customer_list) + len(n_depot_list)
    cnt = 0
